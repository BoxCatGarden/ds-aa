...
void huffman::encode(const char* originFile, const char* targetFile) {
	hasError = 0;
	...
	head.comFlag = (enTree.getCharLength() + overhead < oFileSize)?1:0;
...
void huffman::writeFileHead(writer& rWriter, const file_head& head) {
	...
	if (head.divFlag) {
		...
		if (head.comFlag) {
			...
			unsigned char j = head.tree.nodeRecordLength;  //no more than 64
...
void huffman::readFileHead(reader& rReader, file_head& head) {
	...
	if (head.divFlag = flag&_bit[0]) {
		...
		if (head.comFlag = flag&_bit[1]) {
			...
			if (i = flag&7) head.tree.nodeRecordLength = 1<<(i-1);
		}
	}
}
