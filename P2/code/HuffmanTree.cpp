...
HuffmanTree::HuffmanTree(reader& rReader):root(NULL), table(NULL), charLength(0) {
	/* count frequence*/
	...
	unsigned short g_k;
	{
		...
		if (j == 1) {
			info.length = 1;
			info.record = new unsigned char[1];
			info.record[0] = ((nodeLeaf64*)nodes[0])->c;
			delete[] frequence;
			delete[] nodes;
			return;
		}
		...
		info.nodeRecordLength = g_k = (j&3?(j>>2)+1:(j>>2));
		info.length = j + g_k;
		info.record = new unsigned char[info.length]; memset(info.record, -1, info.length);
	}
...
calCL:
	/* calculate charLength*/
	{
		...
		if (bitCounter) ++charLength;
		delete[] frequence;
	}
}

HuffmanTree::HuffmanTree(reader& rReader, unsigned char maxNodeRecordLength):root(NULL), table(NULL), charLength(0) {
