...
class code;
struct treeInfo {
	const unsigned char* record;  //the Huffman-tree printed in pre-order
...
class HuffmanTree  
{
	...
	unsigned char _lastSigBitNum;  //the number of the significant bits of the last encoded char, where 0 means it's entirely significant

	unsigned char* record;  //the Huffman-tree printed in pre-order
	unsigned short recordLength;  //the length of record
	unsigned char nodeRecordLength;  //the length of the node-record which is a part of the record

	/* initialize*/
	//HuffmanTree():root(NULL),table(NULL),charLength(0),_lastSigBitNum(0),record(NULL),recordLength(0),nodeRecordLength(0) {}
public:
	...
	*/
	...
	//get the number of the significant bits of the last encoded char, where 0 means it's entirely significant
	unsigned char lastSigBitNum() const {return _lastSigBitNum;}

	//get the tree-info
	void getTreeInfo(treeInfo& infoB) const {infoB.record=record,infoB.length=recordLength;infoB.nodeRecordLength=nodeRecordLength;}
